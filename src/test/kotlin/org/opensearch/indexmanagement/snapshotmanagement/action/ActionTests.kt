...
class ActionTests : OpenSearchTestCase() {
    ...
    fun `test delete action name`() {
        assertNotNull(DELETE_SM_ACTION_TYPE.name())
        assertEquals(DELETE_SM_ACTION_TYPE.name(), DELETE_SM_ACTION_NAME)
    }

    fun `test index action name`() {
        assertNotNull(INDEX_SM_ACTION_TYPE.name())
        assertEquals(INDEX_SM_ACTION_TYPE.name(), INDEX_SM_ACTION_NAME)
    }
