...
class ActionValidation(
    ...
    fun validate(actionName: String, indexName: String): ValidationResult {
        ...
        val validation = when (actionName) {
            ...
            "close" -> ValidateClose(settings, clusterService, jvmService).execute(indexName)"replica_count" -> ValidateReplicaCount(settings, clusterService, jvmService).execute(indexName)
            else -> {
