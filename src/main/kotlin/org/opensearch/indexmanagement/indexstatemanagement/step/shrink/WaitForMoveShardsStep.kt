...
class WaitForMoveShardsStep(private val action: ShrinkAction) : ShrinkStep(name, true, true, false) {
    ...
    override suspend fun wrappedExecute(context: StepContext): WaitForMoveShardsStep {
        ...
        val numPrimaryShards = context.clusterService.state().metadata.indices[indexName]?.numberOfShards ?: throw IllegalStateException("numberOfShards should not be null")
    ...
    private fun getNumShardsInSync(shardStats: Array<ShardStats>, state: ClusterState, indexName: String): Int {
        val numReplicas = state.metadata.indices[indexName]?.numberOfReplicas ?: throw IllegalStateException("numberOfReplicas should not be null")
