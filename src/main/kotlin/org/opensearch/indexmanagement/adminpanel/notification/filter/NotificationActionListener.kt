...
class NotificationActionListener<Request : ActionRequest, Response : ActionResponse>(
    ...
    private fun notifyOperationComplete(
        ...
        val searchParam = SearchParams(
            DEFAULT_PAGINATION_SIZE, 0, "lron_config.priority", SORT_ORDER_DESC, DEFAULT_QUERY_STRING
        )
        client.execute(
            GetLRONConfigAction.INSTANCE,
            GetLRONConfigsRequest(
                searchParam,
                arrayOf(
                    getDocID(), getDocID(taskId = taskId),
                    getDocID(actionName = action)
                )
            ),
